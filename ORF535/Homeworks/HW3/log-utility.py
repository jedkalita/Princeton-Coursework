...
target_returns = np.array([1.05, 1.06, 1.07, 1.08, 1.09, 1.1, 1.105]) #we will optimize for each one
'''target_returns = list()
...
'''for i in range(len(returns_data)):
    returns_data[i] = returns_data[i] - 1'''
print(returns_data)
print(risk_data)

#now the code to calculate max expected log returns from the generated scenarios
'''p_t = np.ones(len(p))
for i in range(len(p)):
    p[i] = p[i] - 1'''
log_p = np.log(p) #take the log of the expected returns produced by the scenarios for each
#asset
print(p)
#print(p_t)
print(log_p)
exp_log_ret = log_p * w #the expected log return of the maximum
log_risk = quad_form(w, covar) #the corresponding risk

log_opt = Problem(Maximize(exp_log_ret),
                  [sum_entries(w) == 1,
                  w >= 0])
log_opt.solve()
log_ret_max = exp_log_ret.value
log_risk_max = sqrt(log_risk).value
print(log_ret_max)
print(log_risk_max)
...
plt.plot(log_risk_max, log_ret_max, 'x')
#ax.annotate('(%s, %s)' % log_risk_max % log_ret_max)
X = np.ones(1)
X[0] = log_risk_max
Y = np.ones(1)
Y[0] = log_ret_max
