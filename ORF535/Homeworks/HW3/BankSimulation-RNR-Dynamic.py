...
asset_investments_rnr = list() #beginning of each month how much money is invested - long tlt
...
for i in range(len(R_TBill_beg)):
    if i == 0: #for the very first period it will be just return from T-Bills
        ...
        spread = (AGG_R[i - 1] + RNR_R[i - 1]) - TBill_R[i - 1] #the spread
        lev_ratio = float((RNR_R[i - 1] + AGG_R[i - 1])/ TBill_R[i - 1])
