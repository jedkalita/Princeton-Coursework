...
num_red = 0
inverted_yield = 0
y = 1 #dynamic overlay strategy of investment policy, we start with y = 2 (for the first period)
...
asset_investments_tlt = list() #beginning of each month how much money is invested - long tlt
...
for i in range(len(R_TBill_beg)):
    if i == 0: #for the very first period it will be just return from T-Bills
        ...
        assets_returns_prev_year = asset_investments[i - 1] * AGG_R[i - 1] + \
                                   asset_investments_tlt[i - 1] * TLT_R[i - 1]
        ...
        spread = (AGG_R[i - 1] + TLT_R[i - 1]) - TBill_R[i - 1] #the spread
        lev_ratio = float((TLT_R[i - 1] + AGG_R[i - 1])/ TBill_R[i - 1])
