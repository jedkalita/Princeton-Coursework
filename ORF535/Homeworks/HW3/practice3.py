...
def optimal_portfolio(returns):
    n = len(returns)
    print(n)
    returns = np.asmatrix(returns) #(3, 1000)
    print(returns.shape)
    ...
    S = opt.matrix(np.cov(returns)) #(3 x 3)
    print(S)
    ...
    print(G)
    h = opt.matrix(0.0, (n, 1)) #zero column matrix of no of rows = n
    print(h)
    A = opt.matrix(1.0, (1, n)) #one row matrix of no of columns = n
    print(A)
    b = opt.matrix(1.0) #matrix with one row and one column and containing 1
    print(b)
